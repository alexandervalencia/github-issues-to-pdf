...
function filterIssuesByYear(issues, year, datedIssues) {
  issues.forEach(issue => {
    ...
    const notClosedOrClosedAfterDate =
      issue.created.match(regexIssueYear) <= year &&
      (!issue.closed || issue.closed.match(regexIssueYear) >= year);
      ...
      getIssues(
        ...
        issues => {
          if (!config.year.issueYear) {
            ...
            const datedIssues = filterIssuesByYear(
              issues,
              config.year.issueYear,
              []
            );
        ...
        (err, res) => {
          process.stdout.write(
            `\n${chalk.yellow(
              'All issues have finished rendering, have a nice day!'
            )}\n\n`
          );
        }
      );
...
function getAllOrgRepos(config, collectedRepos, callback) {
  github.repos.getForOrg(
    ...
    (err, res) => {
      if (err) {
        process.stdout.write(
          `${chalk.bgRed('ERROR:')} The account ${chalk.yellow(
            '['
          )}${chalk.yellow(config.accountInfo.accountName)}${chalk.yellow(
            ']'
          )} couldn't be found, double-check the name and try again.\n`
        );
...
function getIssues(config, repo, collectedIssues, callback) {
  github.issues.getForRepo(
    ...
    (err, res) => {
      if (err) {
        process.stdout.write(
          `${chalk.bgRed('ERROR:')} Either the account ${chalk.yellow(
            '['
          )} ${chalk.yellow(config.accountInfo.accountName)} ${chalk.yellow(
            ']'
          )} or the repository ${chalk.yellow('[')} ${chalk.yellow(
            repo
          )} ${chalk.yellow(
            ']'
          )} couldn\'t be found, double-check the names and try again.\n`
        );
...
function getMultipleRepos(config) {
  async.eachSeries(config.multipleRepos, (repo, callback) => {
    config.page = 1;
...
function getSingleRepo(config) {
  getIssues(config, config.accountInfo.repoName, [], issues => {
    if (!config.year.issueYear) {
...
function manageGitHub(config) {
  ...
  if (
    config.accountInfo.accountType === 'organization' &&
    config.accountInfo.howManyRepos === 'all'
  ) {
    getAllForOrg(config);
  } else if (
    config.accountInfo.accountType === 'user' &&
    config.accountInfo.howManyRepos === 'all'
  ) {
...
function promptForAccountInfo() {
  return inq.prompt([
    ...
    },
    {
      ...
      message:
        'Please enter the name of the account you would like to query (required):',
      name: 'accountName',
      type: 'input',
      validate: answer => {
        if (answer == '') {
...
function promptForRepos(sep, repoListForPrompt, repoArray) {
  ...
  return inq
    .prompt([
      ...
      {
        message: answers => {
          return `Current repository list: ${chalk.yellow(
            repoListForPrompt,
            sep,
            answers.repoName
...
function promptForToken() {
  return inq
    .prompt([
      {
        message:
          "Your GitHub Personal Access Token file doesn't exist yet, enter your token here and I'll save it for you:",
        name: 'id',
        type: 'input',
        validate: value => {
...
function promptForYear() {
  return inq.prompt([
    {
      message:
        'Are you searching for issues that were open during a specific year?',
      name: 'searchByDate',
      type: 'confirm',
    },
    {
...
function queueIssuesForRender(issues, callback) {
  async.mapLimit(issues, 15, renderIssue, (err, res) => {
    if (err) {
      throw err;
    }
    ...
    if (issues[0] != undefined) {
      process.stdout.write(
        `\n${chalk.yellow('All issues from ')} ${chalk.cyan('[')} ${chalk.cyan(
          issues[0].repo
        )} ${chalk.cyan(']')} ${chalk.yellow(' have finished rendering.')}\n\n`
      );
    }
...
async function renderIssue(issue) {
  if (issue != undefined) {
    ...
    await page.goto(
      `https://github.com/${issue.owner}/${issue.repo}/issues/${issue.num}`,
    ...
    await page.pdf({
      path: `${localPath}/${issue.owner}_${issue.repo}_${issue.created.match(
        regexFileDate
      )}_issue${issue.num}.pdf`,
      format: 'letter',
    });

    process.stdout.write(
      `File created at [ ${localPath}/${issue.owner}_${
        issue.repo
      }_${issue.created.match(regexFileDate)}_issue${issue.num}.pdf ]\n`
    );

    await browser.close();
  } else {
    process.stdout.write(
      `\n${chalk.yellow('No issues found in')} ${chalk.cyan('[')}${chalk.cyan(
        issue.repo
      )}${chalk.cyan(']')}.\n\n`
    );
